...
{
 int    n;			        // last index
 double sum_k;		// outer and inner sum
 double time1=0.0, time2=0.0, time3=0.0, tstart;  // time measurement
...
//
 ...
 for ( int k = 1; k <= n; ++k)
  {
    double sum_i = 0.0;			// initialize inner sum
...
//
//	fast version [Hr. Andreas Reinhart]
//
 tstart = clock();
 sum_k = 0.0;
...
//    sum_k = sum_k + t_k/n;		// sum_k is bounded
 ...
 cout << endl;
 cout << "  c) Double-Sum (" << n << ") = " << sum_k;
 cout << "  in  " << time3/CLOCKS_PER_SEC << " sec." << endl;
 cout << endl;
 return 0;
}
