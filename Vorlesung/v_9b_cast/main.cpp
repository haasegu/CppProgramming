...
void pointer_wrong_downcast_C(Employee  simple)
{
    ...
    sale_Ptr->setComission(0.06f);		// nunmehr formal (ohne vorherige Zeile) machbar, aber katastrophal da diese Methode in Employee nicht existiert
...
void pointer_wrong_downcast_dynamic(Employee  simple)
{
    ...
    cout << endl << "         jetzt kracht es" << endl;
    salePtr->setComission(0.06f);		// nunmehr formal (ohne vorherige Zeile) machbar, aber katastrophal da diese Methode in Employee nicht existiert
    cout << salePtr->payment() << endl;
    cout << *salePtr << endl;
    return;
}
...
//!
//! @param[in] aSale Instance of derived class
