...
//	Version 7_dc: try it with a discontinuous function dc(x) ==> segmentation fault, but solved in Bisect(...)
//
...
double dc(const double x)		// declaration and
{
    return x>=std::sqrt(2.0)? -x : 2.0*x ;    //   definition of function t(x)
}
...
int main()
{
    ...
    array<string, 5> ssf{"f(x) :=  sin(x) - x/2",
                         "g(x) := (1.234567-x)*(x+0.987654)",
                         "h(x) :=  3.0 - exp(x)",
                         "t(x) :=  1-x*x",
                         "d(x) :=  x>=std::sqrt(2.0)? -x : 2.0*x" };

// now with array of functions
    array< std::function<double(double)>, 5> vff{f, g, h, t, dc};

    // map  char to index
    map<char, int> mmf{{'f', 0}, {'g', 1}, {'h', 2}, {'t', 3}, {'d',4}};

    // map  char to function
    map<char, std::function<double(double)> > mapff{{'f', f}, {'g', g}, {'h', h}, {'t', t}, {'d',dc} };
              ...
              const double a, const double b, const double eps)
{
    ...
    cout << a << "  " << b << endl;
    //if ( std::abs(fc) < eps ) {
    if ( std::abs(fc) < eps || std::abs(b-a) < 1e-14) {   // Modification for non-continuous functions
